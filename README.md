...
- `dataset_node.py`: 고정 46노드 키보드 그래프 데이터셋 생성 스크립트(전결합 그래프).
- `dataset_stroke.py`: 키 입력 시퀀스용 그래프 데이터셋 생성 스크립트(npz의 `x`, `iki` 사용).
- `feature_extraction.py`: CoAtNet 기반 특징 추출 파이프라인/모델 정의(가변 크기 대응).
- `mel_spectrogram.py`: WAV에서 타건음을 분리하고 멜-스펙트로그램 이미지를 생성하는 배치 스크립트.
